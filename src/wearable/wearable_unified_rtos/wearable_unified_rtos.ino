...
// Date: 2026-05-13 | FreeRTOS: 2026-05-20
//
// ── Chế độ hoạt động ─────────────────────────────────────────────
...
//
// ── FreeRTOS Task Map ─────────────────────────────────────────────
//   TaskIMURead   Core1 Pri5 Stack3072 — Đọc MPU6050 100Hz → imuQueue
//   TaskEdgeAI    Core1 Pri4 Stack4096 — Inference + Debounce + LED
//   TaskNetworkWeb Core0 Pri3 Stack6144 — Web server + HTTPS Upload
//
...
bool initEspNow() {
  ...
  Serial.println("[ESP-NOW] Đã đăng ký peer Trạm chính thành công.");
  return true;
}
...
// stateMutex: bảo vệ operationMode và state khi đọc/ghi từ nhiều task
static SemaphoreHandle_t stateMutex = nullptr;

// ── FreeRTOS Stack Sizes ──────────────────────────────────────────
#define STACK_IMU_READ    4096
#define STACK_EDGE_AI     4096
#define STACK_NET_WEB     6144
...
void TaskIMURead(void *pvParam);
void TaskEdgeAI(void *pvParam);
void TaskNetworkWeb(void *pvParam);
...
void handleTrigger() {
  if (operationMode == INGESTION && state == READY) {
    xQueueReset(imuQueue); // Xóa sạch dữ liệu hàng đợi cũ
    samplesCollected = 0;
    sampleTimer = micros();
    state = COLLECTING;
...
void handleCalibrate() {
  ...
  xQueueReset(imuQueue); // Xóa sạch dữ liệu hàng đợi cũ để tránh lệch offset
...
void setup() {
  ...
  ledOff();
  Serial.println("\n[MAIN] === WEARABLE UNIFIED v2 — FreeRTOS ===");
  ...
  Serial.println("[WIFI] Thiết lập WIFI_AP_STA...");
  WiFi.mode(WIFI_AP_STA);
  // Khởi động SoftAP cố định ở Kênh 1 để ESP-NOW truyền nhận ổn định đồng kênh
  WiFi.softAP("Wearable_AP", "12345678", 1);
  Serial.println("[WIFI] SoftAP 'Wearable_AP' đã mở ở Kênh 1.");

  Serial.printf("[WIFI] Kết nối STA tới %s...\n", EI_WIFI_SSID);
  ...
  // ── FreeRTOS: Tạo primitives ──────────────────────────────────
  imuQueue  = xQueueCreate(IMU_QUEUE_LEN, sizeof(ImuData));
  stateMutex = xSemaphoreCreateMutex();
  configASSERT(imuQueue  != nullptr);
  configASSERT(stateMutex != nullptr);

  // ── FreeRTOS: Spawn tasks ─────────────────────────────────────
  xTaskCreatePinnedToCore(TaskIMURead,    "IMURead",  STACK_IMU_READ, nullptr, 5, nullptr, 1);
  xTaskCreatePinnedToCore(TaskEdgeAI,     "EdgeAI",   STACK_EDGE_AI,  nullptr, 4, nullptr, 1);
  xTaskCreatePinnedToCore(TaskNetworkWeb, "NetWeb",   STACK_NET_WEB,  nullptr, 3, nullptr, 0);

  Serial.println("[RTOS] Tasks started — IMURead/EdgeAI on Core1, NetWeb on Core0");
}
...
// ─── Task 1: IMU Read ────────────────────────────────────────────
// Core1 | Priority 5 (cao nhất — không được bị preempt)
// Đọc MPU6050 chính xác mỗi 10ms (100Hz), cập nhật web snapshot,
// đẩy ImuData vào imuQueue để TaskEdgeAI tiêu thụ.
void TaskIMURead(void * /*pvParam*/) {
  const TickType_t xInterval = pdMS_TO_TICKS(10); // 100Hz
  ...
  for (;;) {
    ...
    if (d.status == SENSOR_OK) {
      // Cập nhật web snapshot (volatile, safe atomic trên ESP32)
      web_aX = d.aX; web_aY = d.aY; web_aZ = d.aZ;
      web_gX = d.gX; web_gY = d.gY; web_gZ = d.gZ;
      web_totalG = sqrtf(d.aX * d.aX + d.aY * d.aY + d.aZ * d.aZ);

      // Đẩy vào queue (không block — nếu đầy thì bỏ item cũ nhất)
      if (uxQueueSpacesAvailable(imuQueue) == 0) {
        ImuData discard;
        xQueueReceive(imuQueue, &discard, 0);
      }
      xQueueSend(imuQueue, &d, 0);
    }
...
// ─── Task 2: Edge AI Inference ───────────────────────────────────
// Core1 | Priority 4
// Nhận ImuData từ imuQueue, lấp đầy sliding window inferBuf,
// chạy Edge Impulse inference + bộ lọc Debounce/Cooldown + LED.
void TaskEdgeAI(void * /*pvParam*/) {
  ...
  for (;;) {
    // Kiểm tra chế độ trước khi lấy mẫu từ queue để tránh tranh chấp hàng đợi ở chế độ Ingestion
    OperationMode curMode = INFERENCE;
    if (xSemaphoreTake(stateMutex, pdMS_TO_TICKS(5)) == pdTRUE) {
      curMode = operationMode;
    ...
    if (curMode != INFERENCE) {
      vTaskDelay(pdMS_TO_TICKS(100)); // Nhường hoàn toàn queue cho Ingestion
      continue;
    }

    ImuData d;
    // Block tối đa 20ms chờ data từ TaskIMURead
    if (xQueueReceive(imuQueue, &d, pdMS_TO_TICKS(20)) != pdTRUE) {
      continue;
    }

    // Ghi vào sliding window (slot cuối cùng của inferBuf)
        ...
        if (numpy::signal_from_buffer(
          ...
          if (run_classifier(&sig, &res, false) == EI_IMPULSE_OK) {
            ...
            for (size_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
              ...
              if (strcmp(l, "fall") == 0)       inferResult.fall = v;
              else if (strcmp(l, "idle") == 0)  inferResult.idle = v;
              else if (strcmp(l, "walk") == 0)  inferResult.walk = v;
              if (v > best) {
                best = v;
            ...
            if (inferResult.fall >= FALL_ALERT_THRESHOLD) {
              ...
              if (fallConfirm >= FALL_CONFIRM_SLICES &&
                ...
                // --- PHÁT ESP-NOW CẢNH BÁO ---
                FallAlertPacket packet;
                packet.alertType = 0xFA;
                packet.fallCount = inferResult.fall_count;
                packet.confidence = inferResult.fall;
                packet.timestamp = millis();

                for (int i = 0; i < 3; i++) {
                  esp_now_send(MAIN_BOARD_MAC, (uint8_t *)&packet, sizeof(packet));
                  vTaskDelay(pdMS_TO_TICKS(5)); // Chờ nhẹ giữa các lần phát lặp để giảm va chạm sóng
                }
                logPrintf("[ESP-NOW] Đã phát gói tin cảnh báo té ngã tới Trạm chính (3x).\n");
                // ------------------------------
            ...
            Serial.printf("[INF] fall:%.2f idle:%.2f walk:%.2f / X:%.2f Y:%.2f Z:%.2f -> %s%s "
                          "[DSP:%dms CLS:%dms]\n",
                          inferResult.fall, inferResult.idle, inferResult.walk,
                          web_aX, web_aY, web_aZ,
                          inferResult.top_label,
...
// ─── Task 3: Network & Web Server ────────────────────────────────
// Core0 | Priority 3
// Xử lý toàn bộ hoạt động mạng: Web server handleClient(),
// INGESTION state machine (Calibrating / Collecting / Uploading).
// Chạy trên Core0 — mọi blocking network không ảnh hưởng Core1.
void TaskNetworkWeb(void * /*pvParam*/) {
  static unsigned long ledTimer = 0;
  static bool ledState = false;
  ...
  for (;;) {
    ...
    if (curMode == INFERENCE) {
      // Ở chế độ suy luận chỉ cần phục vụ web client
      server.handleClient();
    ...
    case READY:
      ...
      // Nút boot phần cứng kích hoạt thu mẫu
    ...
    case CALIBRATING: {
      ...
      // Lấy mẫu từ imuQueue thay vì đọc I2C trực tiếp trên Core 0
      ImuData d;
      if (xQueueReceive(imuQueue, &d, pdMS_TO_TICKS(15)) == pdTRUE) {
        if (d.status == SENSOR_OK) {
          calSumAX += d.aX; calSumAY += d.aY; calSumAZ += d.aZ;
          calSumGX += d.gX; calSumGY += d.gY; calSumGZ += d.gZ;
          calCount++;
        }
    ...
    case COLLECTING: {
      ...
      // Lấy mẫu từ imuQueue thay vì đọc trực tiếp
      ImuData d;
      if (xQueueReceive(imuQueue, &d, pdMS_TO_TICKS(15)) == pdTRUE) {
    ...
    case DONE_OK:
      ledOff();
      lastUploadOk = true;
      snprintf(lastUploadTime, sizeof(lastUploadTime), "%s OK", currentSampleName);
    ...
    case DONE_ERR:
      ledOff();
      lastUploadOk = false;
      snprintf(lastUploadTime, sizeof(lastUploadTime), "%s ERR", currentSampleName);
      logPrintf("[EI] THẤT BẠI! '%s'\n", currentSampleName);
      if (autoRunActive) autoRunActive = false;
      if (manualUploadActive) {
        manualUploadActive = false;
        augmentMode = manualSavedAugMode;
        jitterMode  = manualSavedJitter;
    ...
    server.handleClient();
    vTaskDelay(pdMS_TO_TICKS(5)); // nhường CPU giữa các vòng lặp web
  }
}
...
// Tất cả xử lý thực sự diễn ra trong 3 tasks bên trên.
